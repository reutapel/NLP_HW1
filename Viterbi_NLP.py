...
class viterbi(object):
    ...
    def viterbi_sentence(self, word_tag_list, sentence_index):
        ...
        for k in range(1, number_of_words+1):
            ...
            current_word_possible_tags, current_unseen_word, current_unk_word =\
                self.possible_tags(current_word, is_cur_word=True)
            if current_unseen_word:  # never the seen the word in the train set
            ...
            if current_unk_word:
                unk_word_list.append(k - 1)

            u_possible_tags, unseen_word_u, unk_word_u = self.possible_tags(second_word)
            for u in u_possible_tags:
                for v in current_word_possible_tags:
                    calc_max_pi = float("-inf")
                    calc_argmax_pi = -1
                    w_possilbe_tags, unseen_word_w, unk_word_w = self.possible_tags(first_word)
                    for w in w_possilbe_tags:
        ...
        for k in range(number_of_words):
            if k in unk_word_list:
                ...
                if u_v in self.transition_tag_dict:
                    if sen_word_tag_predict[k] not in self.transition_tag_dict[u_v]['TOP'][:2]:
                        sen_word_tag_predict[k] = self.transition_tag_dict[u_v]['TOP'][0]
                elif tag_k_1 in self.transition_tag_dict:
                    if sen_word_tag_predict[k] not in self.transition_tag_dict[tag_k_1]['TOP'][:2]:
        ...
        if word == '*':
            ...
            if word not in self.word_tag_dict:
                ...
                if word.lower() in self.word_tag_dict:  # if the word in unseen, but the lower case of the word is seen
                    ...
                    if tags_list != self.most_common_tags and is_cur_word:
                ...
                # word contains number instances or word is a number
                    ...
                    if is_cur_word:
                        self.digit_case += 1
                # word contains only upper letters and contain upper and lower
                ...
                if tags_list == self.most_common_tags:
                    tags_list = self.find_tags_for_unseen_singular_nouns(word)
                    if tags_list != self.most_common_tags and is_cur_word:
                        self.single_case += 1
                        # print('use single nouns case for word {}'.format(word))
            ...
            if tags_list == self.most_common_tags:
                ...
                if is_cur_word:
                    self.unk_case += 1
                unk_word = True  # none of the conditions are good for this word
                if len(word) > 2:
                    if word[-3:] == 'ing':
    ...
    def find_tags_for_unseen_plural_nouns(self, word):
        ...
        if 'NN' in tags_list and tags_list != self.most_common_tags:  # add VBZ for verbs like wish-wishes
            tags_list = ['NNS']
        elif 'NNP' in tags_list and tags_list != self.most_common_tags:
            tags_list = ['NNPS']
    ...
    def find_tags_for_unseen_singular_nouns(self, word):
        ...
        if 'NNS' in tags_list and tags_list != self.most_common_tags:  # add VB for verbs like wish-wishes
            tags_list = ['NN']
        elif 'NNPS' in tags_list and tags_list != self.most_common_tags:
            tags_list = ['NNP']
